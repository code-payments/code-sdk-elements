...
import { LoginRequest, UserAgent } from "../../utils"
...
const loginRequest = ref(LoginRequest.fromPayload(props.payload));
...
function getAppUrl() {
  const payload = loginRequest.value.toPayload();
...
function onClose () {
  open.value = false;
  loginRequest.value.closeStream();
...
channel.on("clientRejectedLogin", () => { hasRejected.value = true; });
...
channel.on("afterInvoke", async () => {
  ...
  if (shouldRecreateRequest) {
    loginRequest.value = LoginRequest.fromPayload(props.payload, {
  ...
  loginRequest.value.openStream(channel);
...
onMounted(() => {
  ...
  if (loginRequest.value && !loginRequest.value.kikCode) {
    loginRequest.value.generateKikCode()
  }
});
                    ...
                    <div v-else>
                      ...
                      <div class="delay-show-1 pb-10">
                        <div class="pb-5 text-white text-[28px] text-center leading-tight
                          tracking-tighter font-medium">Use Code App to Login to {{ loginRequest.intent.domain }}
                        </div>
